...
export const loginApi = async (credentials: LoginCredentials): Promise<AuthResponse> => {
  try {
    ...
    return {
      user: { id: data.user.id, email: data.user.email, name: data.user.nome, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
      tokens: { accessToken: data.accessToken, refreshToken: data.refreshToken, expiresAt: Date.now() + 7 * 24 * 60 * 60 * 1000 },
    };
  } catch (error: any) {
...
export const registerApi = async (credentials: RegisterCredentials): Promise<AuthResponse> => {
  try {
    const response = await fetch(`${API_URL}/auth/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ nome: credentials.nome, email: credentials.email, senha: credentials.senha, dataNascimento: credentials.dataNascimento, cidade: credentials.cidade }),
    });
    const data = await response.json();
...
export const refreshTokenApi = async (refreshToken: string): Promise<AuthTokens> => {
  try {
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(data.message || 'Erro ao renovar token');
    return { accessToken: data.accessToken, refreshToken: data.refreshToken, expiresAt: Date.now() + 7 * 24 * 60 * 60 * 1000 };
  } catch (error: any) {
